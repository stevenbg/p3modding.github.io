...
fleet crew at `0x004F7E27`). Crew morale is the word at `ship+0x3E`. Everything the game
decides about crewing - how many a ship needs, how many it wants, how many the tavern
will hand over - comes from two byte tables and two routines.
...
The full-crew table is a different figure: the speed computation grants a bonus of
`(crew - full) * 400` internal points only while crew exceeds it (`0x518377`,
`0x5184B3`, `0x518571` read it), and the hire cap below fills toward it. Note the Cog
...
citizens, and a partial dismissal costs crew morale.
